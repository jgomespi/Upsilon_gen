...
int main() { 
  ...
  pythia.readString("Bottomonium:all=on"); // esta é a única flag de processo ligada, ativa a produção de todas as ressonâncias de bottomonium
  ...
  TH2D* mu_pt_eta = new TH2D("mu_pt_eta", "p_{T} x #eta de cada muon",100,0,30,100,-4.,4.);
  ...
  int nev(10000); // Número total de eventos gerados
  ...
  for (int iEvent = 0; iEvent < nev; ++iEvent) {
    ...
    if (munIndex!=0 && mupIndex!=0) {
      ...
      if (IPLUS*pythia.event[munIndex].pT() > 3.5      && IPLUS*pythia.event[mupIndex].pT() > 3.5 &&
        abs(IPLUS*pythia.event[munIndex].eta()) < 2.1 && abs(IPLUS*pythia.event[mupIndex].eta()) < 2.1) {
        ncutPLUS++;
        UpsilonCutPt_PLUS->Fill(IPLUS*pythia.event[indexUpsilon].pT());          
      }      
      // Verificar se o Upsilon polarizado com alpha=-1 está na região de aceptância
      if (IMINUS*pythia.event[munIndex].pT() > 3.5      && IMINUS*pythia.event[mupIndex].pT() > 3.5 &&
        abs(IMINUS*pythia.event[munIndex].eta()) < 2.1 && abs(IMINUS*pythia.event[mupIndex].eta()) < 2.1) {
        ncutMINUS++;
  ...
  AcceptPLUS->SetTitle("Acceptance ");
  AcceptPLUS->GetXaxis()->SetTitle("p_{T}^{#Upsilon} (GeV)");  
  AcceptPLUS->GetYaxis()->SetTitle("A^{#Upsilon}");    
  AcceptPLUS->SetMarkerStyle(2);
  Accept->SetMarkerStyle(3);  
  AcceptMINUS->SetMarkerStyle(4);
  //AcceptPLUS->SetLineColor(2);
  //Accept->SetLineColor(3);  
  //AcceptMINUS->SetLineColor(4);

  auto legend = new TLegend(0.1,0.75,0.3,0.9);
  legend->SetHeader("Polarization","C"); // option "C" allows to center the header
  ...
  // Desenhar e salvar gráfico da regio de acceptância
  TCanvas* c2 = new TCanvas("c2","",800,800);
  mu_pt_eta->GetXaxis()->SetTitle("p_{T} (GeV)");
  mu_pt_eta->GetYaxis()->SetTitle("#eta");
  mu_pt_eta->Draw();
  TLine *line1 = new TLine(0.,-2.1,30.,-2.1);
  TLine *line2 = new TLine(0.,2.1,30.,2.1);
  TLine *line3 = new TLine(3.5,-2.1,3.5,2.1);
  ...
  TText *t = new TText(22.,-2.,"Acceptance region");
  t->SetTextColor(kRed);
  t->SetTextFont(43);
  t->SetTextSize(20);
